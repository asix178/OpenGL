...
glm::vec3 move2(0.0f, 0.0f, 6.0f);
glm::vec3 aLightPos = glm::vec3(0.0f, 2.0f, -8.0f);
...
float diffuseStrength = max(dot(LightDir, Normal), 0.7);
...
vec3 FinalLightResult = 1/distance * (ambient + diffuse + specular) * 4;
...
//Dane œcian zewnêtrznych
glm::vec3 positionsOfWalls[2] = {
...
int main()
{
    ...
    for (int i = 0; i < 5; i++)
    {
        enemies[i].pos = glm::vec3((rand() % 19) - 9, 0, i * randomFloatInRange(5, 7) - 30);
        enemies[i].scale = 1.0f + (rand() / (float)RAND_MAX);
        enemies[i].isAPoint = false;
    }
    enemies[5].pos = glm::vec3((rand() % 19) - 9, 0, randomFloatInRange(5, 20) - 30);
    enemies[5].scale = 1.0f + (rand() / (float)RAND_MAX);
    enemies[5].isAPoint = true;

    //game loop
    while (!glfwWindowShouldClose(window))
    {
        clock_t current = clock();
        float deltaTime = (float)(current - previous) / CLOCKS_PER_SEC;
        previous = current;
        timer += deltaTime;
        ...
        glm::mat4 projMat = glm::perspective(glm::radians(45.0f), (float)SCREEN_WIDTH / (float)SCREEN_HEIGHT, 0.01f,
            100.0f);
        ...
        for (int i = 0; i < 2; i++)
        {
            modelMat = glm::translate(glm::mat4(1.0f), positionsOfWalls[i]);
            modelMat = glm::scale(modelMat, glm::vec3(1.0f, 7.0f, 25.0f));
            ...
            glUniform3f(glGetUniformLocation(shaderProgram, "object_mat.ambient"), 0.5f, 0.5f, 0.5f); // object ambient
            glUniform3f(glGetUniformLocation(shaderProgram, "object_mat.diffuse"), 1.0f, 1.0f, 1.0f); // object diffuse
            glUniform3f(glGetUniformLocation(shaderProgram, "object_mat.specular"), 1.0f, 1.0f, 1.0f);
            // object specular
        ...
        for (int i = 0; i < 6; i++)
        {
            modelMat = glm::translate(glm::mat4(1.0f), enemies[i].pos);
            modelMat = glm::scale(modelMat, glm::vec3(enemies[i].scale, 2, enemies[i].scale));
            glUniformMatrix4fv(2, 1, GL_FALSE, glm::value_ptr(modelMat));

            glUniform3f(5, camera_position.x, camera_position.y, camera_position.z); // view position

            if (!enemies[i].isAPoint)
            {
                glUniform3f(glGetUniformLocation(shaderProgram, "object_mat.ambient"), 0.3f, 0.3f, 0.3f); // object ambient
                glUniform3f(glGetUniformLocation(shaderProgram, "object_mat.diffuse"), 0.0f, 1.0f, 0.0f); // object diffuse
                glUniform3f(glGetUniformLocation(shaderProgram, "object_mat.specular"), 1.0f, 1.0f, 1.0f);
            }
            else
            {
                glUniform3f(glGetUniformLocation(shaderProgram, "object_mat.ambient"), 0.3f, 0.3f, 0.3f); // object ambient
                glUniform3f(glGetUniformLocation(shaderProgram, "object_mat.diffuse"), 0.0f, 0.0f, 1); // object diffuse
                glUniform3f(glGetUniformLocation(shaderProgram, "object_mat.specular"), 1.0f, 1.0f, 1.0f);
            }

            // object specular
            glUniform1f(glGetUniformLocation(shaderProgram, "object_mat.shininess"), 40); // object shininess
        ...
        glm::mat4 modelMat3 = glm::translate(glm::mat4(1.0f), glm::vec3(move2.x, 0.0f + move2.y, move2.z));
        modelMat3 = glm::scale(modelMat3, glm::vec3(2.0f, 2.0f, 2.0f));
        ...
        glUniform3f(glGetUniformLocation(shaderProgram, "light.position"), aLightPos.x, aLightPos.y, aLightPos.z);
        //light position
        glUniform3f(glGetUniformLocation(shaderProgram, "light.ambient"), 0.7f, 0.7f, 0.7f); // light ambient
        ...
        for (int i = 0;i < 6;i++)
        {
            enemies[i].pos.z += timer / 500 + 0.05f;
            if (checkCollision(enemies[i].pos, glm::vec3(enemies[i].scale, 2, enemies[i].scale), move2, glm::vec3(2.0f, 2.0f, 2.0f)))
            {
                if (enemies[i].isAPoint)
                {
                    points++;
                    enemies[i].pos = glm::vec3((rand() % 19) - 9, 0, randomFloatInRange(5, 15) - 20);
                    enemies[i].scale = 1.0f + (rand() / (float)RAND_MAX);
                }
                else
                {
                    std::cout << "U gathered: " << points * timer << " points!" << std::endl;
                    exit(0);
                }
            }


            if (enemies[i].pos.z >= 10 - enemies[i].scale)
            {
                enemies[i].pos = glm::vec3((rand() % 19) - 9, 0, randomFloatInRange(5, 7) - 20);
                enemies[i].scale = 1.0f + (rand() / (float)RAND_MAX);
            }
        }
    }
...
void processInput(GLFWwindow* window)
{
    ...
    {
        if (!checkCollision(positionsOfWalls[0], glm::vec3(1.0f, 7.0f, 25.0f), move2, glm::vec3(2.0f, 2.0f, 2.0f)))
        {
            move2.x -= 0.1f;
        }
    }
    ...
    {
        if (!checkCollision(positionsOfWalls[1], glm::vec3(1.0f, 7.0f, 25.0f), move2, glm::vec3(2.0f, 2.0f, 2.0f)))
        {
            move2.x += 0.1f;
        }
    }
